...
public class ItemInit {
	...
	public static final RegistryObject<Item> TREAT = ITEMS.register("treat",
			() -> new Item(new Item.Properties().tab(ItemGroup.TAB_FOOD)
					.food(new Food.Builder().effect(() -> new EffectInstance(Effects.MOVEMENT_SLOWDOWN, 100, 1), 1.0f ).effect(() -> new EffectInstance(Effects.HARM, 1, 1), 0.25f ).nutrition(1).fast().saturationMod(0.1F).meat().build())));
}
